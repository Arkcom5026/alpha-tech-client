...
const FormBranch = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: err?.response?.data?.message || err?.message || fallbackMessage,
      });
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      feedback.actionSuccess(successMessage, `admin-branch:delete:${branchId}:success`);
  ...
  return (
    <>
      ...
      <ConfirmActionDialog
        ...
        description={`ต้องการลบสาขา “${pendingDeleteBranch?.name || ''}” หรือไม่?`}
        confirmLabel="ลบสาขา"
        intent="destructive"
