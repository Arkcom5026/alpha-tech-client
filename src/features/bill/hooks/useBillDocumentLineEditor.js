...
export const useBillDocumentLineEditor = ({ documentKey, saveDocumentLine } = {}) => {
  ...
  const save = useCallback(async (item) => {
    ...
    if (typeof saveDocumentLine !== 'function') {
      return { ok: false, code: 'DOCUMENT_LINE_SAVE_UNAVAILABLE', error: 'เอกสารนี้ยังไม่รองรับการแก้ไขรายการ' };
    }

    if (savingLineKeyRef.current === key) {
      return {
        ok: false,
        code: 'DOCUMENT_LINE_UPDATE_IN_PROGRESS',
    ...
    try {
      const result = await saveDocumentLine({ item, draft });
      if (!result?.ok) {
        setError(result?.error || 'บันทึกข้อความรายการเอกสารไม่สำเร็จ');
        return result;
      }
...
export default useBillDocumentLineEditor;
