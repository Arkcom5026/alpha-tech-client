...
const ListBrandPage = () => {
  ...
  const onDetachBrand = async (link) => {
    ...
    const ok = window.confirm(
      `ต้องการถอด "${brandName}" ออกจากประเภทสินค้า "${selectedProductType?.name || ''}" หรือไม่?`
    );
    if (!ok) return;
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {items.map((brand, index) => {
                      ...
                      return (
                        <tr key={brand.id} className="border-t border-[hsl(var(--ads-border-default))] hover:bg-[hsl(var(--ads-surface-subtle))]">
