...
export const buildConsolidatedBillProjection = (data) => {
  ...
  const saleItems = sale.saleLines.map((line) => {
    ...
    const rawDescription = line.documentDescription || line.description || '';
    ...
    return {
      ...
      documentSuffix: line.documentSuffix || '',
      hasDocumentLine: Boolean(line.documentPrefix || line.documentDescription || line.documentSuffix),
      productName: line.description || rawDescription || 'สินค้า',
      productModel: '-',
      quantity,
      unit: line.unit || 'ชิ้น',
      amount,
