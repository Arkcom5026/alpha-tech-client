...
const CommunicationProfileSettingsPage = () => {
  ...
  const load = async () => {
    ...
    try { setProfiles(await listCommunicationProfiles() || []); }
  ...
  const save = async () => {
    if (!canManage || !draft.displayName.trim() || saving) return;
    setSaving(true); setError('');
    try { await saveCommunicationProfile(draft); setDraft(emptyDraft); await load(); }
    ...
    <MobileWorkspace eyebrow="Branch Communication" title="ช่องทางติดต่อของสาขา" description="ตั้งค่า QR และช่องทางที่ลูกค้าใช้ติดต่อสาขานี้ ข้อมูลทุกชุดแยกตามสาขาปัจจุบัน">
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.7fr)]">
          ...
          {loading ? <p className="text-sm text-slate-500">กำลังโหลด...</p> : profiles.length ? (
            <div className="grid gap-3 sm:grid-cols-2">
              {profiles.map((profile) => {
                ...
                return <button key={profile.id} type="button" onClick={() => edit(profile)} className="flex min-h-24 items-center gap-3 rounded-xl border border-slate-200 p-3 text-left hover:border-emerald-400">
        ...
        <MobileWorkspaceSection title="เพิ่มหรือแก้ไขช่องทาง" description="ใช้ QR payload เมื่อค่าที่เข้ารหัสต่างจากลิงก์หรือชื่อบัญชี">
          ...
          <fieldset disabled={!canManage} className="grid gap-3 disabled:opacity-60">
      ...
      <MobileActionBar>
        <button type="button" onClick={() => setDraft(emptyDraft)} className="rounded-xl border border-slate-300 bg-white px-5 font-black text-slate-700">ล้างฟอร์ม</button>
        <button type="button" disabled={!canManage || !draft.displayName.trim() || saving} onClick={save} className="rounded-xl bg-emerald-700 px-5 font-black text-white disabled:opacity-40">{saving ? 'กำลังบันทึก' : 'บันทึกช่องทาง'}</button>
      </MobileActionBar>
    </MobileWorkspace>
  );
};
