...
const CustomerMoneyReceiveDetailPage = () => {
  ...
  const canCancel = !isCancelled && Number(record.remainingAmount) === Number(record.amount);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4 md:p-6">
      ...
      {cancelError && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700 print:hidden">{cancelError}</div>}
      <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm print:border-0 print:shadow-none">
        <header className="border-b border-slate-200 pb-4 text-center"><div className="flex items-center justify-center gap-2"><h1 className="text-2xl font-bold text-slate-900">เอกสารรับเงินจากลูกค้า</h1>{isCancelled && <span className="rounded-full bg-rose-100 px-3 py-1 text-xs font-bold text-rose-700">ยกเลิกแล้ว</span>}</div><p className="mt-1 text-sm text-slate-500">Customer Money Receive</p></header>
        ...
        <div className="my-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-5 text-center"><div className="text-sm text-slate-500">จำนวนเงินที่รับ</div><div className="mt-1 text-3xl font-bold text-slate-950">฿{formatMoney(record.amount)}</div></div><div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-center"><div className="text-sm text-emerald-700">เงินลูกค้าคงเหลือปัจจุบัน</div><div className="mt-1 text-3xl font-bold text-emerald-950">฿{formatMoney(record.availableBalance)}</div><div className="mt-1 text-xs text-emerald-700">รวมเงินที่ยังพร้อมนำไปใช้ในสาขาปัจจุบัน</div></div></div>
