...
const DOCUMENT_PURPOSE = 'CUSTOMER_MONEY_RECEIPT';
...
const getCustomerMoneyReceiptSnapshot = (record, mode) => {
...
const resolveCustomerMoneyReceiptPresentation = ({ record, mode } = {}) => {
  const presentationSnapshot = getCustomerMoneyReceiptSnapshot(record, mode);
...
const resolveCustomerMoneyReceiptStoreIdentity = ({ record, presentationSnapshot } = {}) => {
  const frozen = presentationSnapshot?.businessSnapshot?.storeIdentity;
  return isObject(frozen) ? frozen : (record?.branch || {});
};

const buildCustomerMoneyReceiptHeader = ({ record, presentation, presentationSnapshot } = {}) => {
  const storeIdentity = resolveCustomerMoneyReceiptStoreIdentity({ record, presentationSnapshot });
...
export {
  ...
  getCustomerMoneyReceiptSnapshot,
  rendererFamilyForMode,
  resolveCustomerMoneyReceiptPresentation,
  resolveCustomerMoneyReceiptStoreIdentity,
};
