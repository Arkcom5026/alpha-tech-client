  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-4 p-3 md:p-5">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 space-y-4">{Object.entries(grouped).map(([saleId, group]) => <section key={saleId} className="overflow-hidden rounded-xl border border-slate-200"><div className="flex justify-between bg-slate-50 px-4 py-3"><div className="font-semibold">ใบส่งของ {group.saleCode}</div><div className="font-bold">฿{money(group.total)}</div></div><div className="divide-y divide-slate-100">{group.lines.map((line) => <div key={line.id} className="grid gap-2 px-4 py-3 md:grid-cols-[1fr_100px_140px]"><div><div className="font-medium">{line.description}</div><div className="text-xs text-slate-500">{line.saleItemType} #{line.saleItemId} · จำนวน {line.quantity}</div></div><div className="text-right text-sm text-slate-500">มูลค่า ฿{money(line.lineAmount)}</div><div className="text-right font-bold text-indigo-800">ตัด ฿{money(line.appliedAmount)}</div></div>)}</div></section>)}</div>
