...
const ReprintCustomerReceiptPage = () => {
  const { id } = useParams()
  ...
  useEffect(() => {
    const updatePrintHeight = () => {
      ...
      if (measuredHeight > 0) {
        document.documentElement.style.setProperty(
          '--customer-receipt-reprint-short-height',
          `${measuredHeight}px`
        )
      }
    }
    ...
    return () => {
      ...
      document.documentElement.style.removeProperty('--customer-receipt-reprint-short-height')
    }
  ...
  const handlePrint = useCallback(() => {
    try {
      ...
      // Keep the receipt visible so the user can retry printing.
    }
  }, [])
  ...
  return (
    <>
      <style>{`
        ...
        @media print {
          ...
          #root {
            ...
            height: ${
              printMode === 'SHORT'
                ? 'var(--customer-receipt-reprint-short-height, auto)'
                : 'auto'
            } !important;
            min-height: ${
              printMode === 'SHORT'
                ? 'var(--customer-receipt-reprint-short-height, 0)'
                : '0'
            } !important;
            margin: 0 !important;
            padding: 0 !important;
        ...
        <div className="mx-auto flex max-w-[210mm] flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="inline-flex items-center justify-center rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700"
            >
              พิมพ์ใบเสร็จ
            </button>
          </div>
      ...
      >
        <div
          ref={printRootRef}
          className={`customer-receipt-reprint-root mx-auto bg-white text-black dark:bg-white dark:text-black ${
            printMode === 'SHORT'
...
export default ReprintCustomerReceiptPage
