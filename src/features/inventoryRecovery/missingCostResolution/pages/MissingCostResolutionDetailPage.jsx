...
const MissingCostResolutionDetailPage = () => {
  ...
  if (detailQuery.isLoading) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500">กำลังโหลดรายละเอียด...</div>;
  }
  ...
  return (
    <section className="space-y-5">
      <Link to={`/${shopSlug}/pos/stock/missing-cost-resolutions`} className="inline-flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-blue-700">
        <ArrowLeft className="h-4 w-4" />กลับรายการต้นทุนขาด
      </Link>

      <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-600">Resolution #{resolution?.id}</p>
            <h1 className="mt-2 text-2xl font-black text-slate-900 md:text-3xl">{resolution?.product?.name || `Product #${candidate?.productId}`}</h1>
            <p className="mt-2 text-sm text-slate-600">สถานะปัจจุบัน: <span className="font-bold text-slate-900">{resolution?.status}</span></p>
          </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <div className="mt-5 space-y-4">
            ...
            {(resolution?.evidenceVersions || []).map((version) => (
              <div key={version.id} className="rounded-2xl border border-slate-200 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div><p className="text-xs font-bold text-slate-400">Version {version.version}</p><p className="mt-1 font-black text-slate-900">{version.sourceType || 'ไม่ระบุประเภทหลักฐาน'}</p></div>
                  <span className="text-lg font-black text-blue-700">{version.proposedUnitCost == null ? '-' : `${formatNumber(version.proposedUnitCost)} บาท`}</span>
                </div>
                <p className="mt-3 text-sm text-slate-600">{version.evidenceSummary || version.rationale || 'ไม่มีรายละเอียดเพิ่มเติม'}</p>
                <div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2"><span>อ้างอิง: {version.sourceReference || '-'}</span><span>สร้างเมื่อ: {formatDate(version.createdAt)}</span></div>
              </div>
            ))}
          </div>
        </article>
        ...
        <aside className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
          ...
          <ol className="mt-5 space-y-4">
            {(auditQuery.data?.items || []).map((event) => (
              <li key={event.eventId} className="border-l-2 border-slate-200 pl-4">
                <p className="text-sm font-black text-slate-900">{event.previousStatus || '-'} → {event.resultingStatus}</p>
                <p className="mt-1 text-xs text-slate-500">{event.actorIdentity} · {formatDate(event.occurredAt)}</p>
