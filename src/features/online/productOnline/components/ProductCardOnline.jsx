  ...
  const onAddToCart = async () => {
    ...
    try {
      if (exist) {
        ...
        if (increaseQty) {
          await increaseQty(key);
        } else if (updateItemQty) {
          await updateItemQty(key, (exist.quantity || 0) + 1);
        } else {
          await addToCart({ ...exist, quantity: (exist.quantity || 0) + 1 });
        }
      } else {
        await addToCart({
          ...item,
      ...
      feedback.actionSuccess(
        exist ? 'เพิ่มจำนวนสินค้าในตะกร้าแล้ว' : 'เพิ่มสินค้าในตะกร้าแล้ว',
  ...
  return (
    ...
    >
      <div className="border rounded-xl shadow bg-white hover:shadow-xl hover:scale-[1.01] transition-all flex flex-col h-[440px] overflow-hidden relative">
        {highlight && (
          <span className="absolute top-2 left-2 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded shadow">Best Price</span>
        )}
        ...
        <div className="flex-1 p-3 text-sm bg-blue-50 flex flex-col justify-start">
          ...
          <div className="text-xs text-gray-500 mt-1">
            <div className="flex justify-between w-full items-start">
              {isReady ? <div className="text-green-600 text-[12px] font-medium pt-1">✅ พร้อมรับที่สาขา</div> : <div />}
              <button
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="my-1">
              <button
                ...
                className={`rounded-md px-3 py-1.5 transition text-sm flex items-center gap-1 disabled:cursor-not-allowed disabled:opacity-60 ${isInCart ? 'bg-green-500 text-white hover:bg-green-600' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
              >
                <ShoppingCart size={16} /> {adding ? 'กำลังเพิ่ม...' : isInCart ? 'เพิ่มอีก' : 'ตะกร้า'}
              </button>
            </div>
          </div>
        </div>
