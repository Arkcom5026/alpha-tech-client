...
export default function PartnerStoreApplicationPage() {
  ...
  const submit = async (event) => {
    ...
    const payload = {
      businessName: formSnapshot.businessName.trim(),
      contactName: formSnapshot.contactName.trim(),
      contactPhone: formSnapshot.contactPhone.trim(),
      contactEmail: formSnapshot.contactEmail.trim().toLowerCase(),
      businessAddress: formSnapshot.businessAddress.trim(),
      requestedStorefrontSlug: formSnapshot.requestedStorefrontSlug.trim() || undefined,
      note: formSnapshot.note.trim() || undefined,
    };
    ...
    try {
      ...
      const submittedApplication = response.data?.data || null;
      setSubmitted(submittedApplication);
      setForm(initialForm);
      feedback.actionSuccess(
        'ส่งใบสมัครร้านพาร์ตเนอร์เรียบร้อยแล้ว',
        `partner-store:application:${submittedApplication?.applicationCode || payload.contactEmail}:submit:success`,
      );
    } catch (requestError) {
    ...
    <main className="min-h-screen bg-[#FDFBF9] px-4 py-8 text-slate-800 md:py-12">
      <section className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl md:p-9">
        <Link
          ...
          className={`text-sm font-bold text-emerald-700 ${mutationBusy ? 'pointer-events-none opacity-50' : ''}`}
        >← กลับหน้าพาร์ตเนอร์</Link>
        ...
        <form className="mt-7 space-y-5" onSubmit={submit}>
          <fieldset disabled={mutationBusy} className="space-y-5 disabled:opacity-70">
            ...
            <button disabled={mutationBusy} className="w-full rounded-xl bg-emerald-600 px-5 py-3.5 text-sm font-black text-white transition hover:bg-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:opacity-60">
