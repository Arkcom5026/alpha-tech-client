...
export default function PartnerStoreOnboardingPage() {
  ...
  const submit = async () => {
    ...
    const confirmedStoreProfile = Boolean(confirmStoreProfile);
    const confirmedOwnerContact = Boolean(confirmOwnerContact);
    const destinationSlug = canonicalSlug;

    if (!confirmedStoreProfile || !confirmedOwnerContact) {
    ...
    try {
      await completePartnerStoreOnboarding({
        confirmStoreProfile: confirmedStoreProfile,
        confirmOwnerContact: confirmedOwnerContact,
      });
      feedback.actionSuccess(
        'ยืนยันข้อมูลเริ่มใช้งานร้านเรียบร้อยแล้ว',
        `partner-store:onboarding:${destinationSlug}:complete:success`,
      );
      ...
      feedback.actionError(
        requestError,
        mutationMessage,
        `partner-store:onboarding:${destinationSlug}:complete:error`,
      );
    } finally {
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-10">
      <section className="mx-auto max-w-3xl rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm md:p-8">
        ...
        <fieldset disabled={interactionBusy} className="mt-6 space-y-3 rounded-2xl bg-emerald-50 p-5 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <button
          type="button"
          onClick={submit}
          disabled={interactionBusy || !confirmStoreProfile || !confirmOwnerContact}
