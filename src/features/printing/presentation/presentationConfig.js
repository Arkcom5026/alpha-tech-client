...
const normalizeLayer = (source) => {
  if (!isObject(source)) return {}
  const spacing = text(source.layout?.spacing, 8).toLowerCase()
  const blockOrder = Array.isArray(source.layout?.blockOrder)
    ? [...new Set(source.layout.blockOrder.map((item) => text(item, 64).toUpperCase()).filter((item) => BLOCK_TYPES.includes(item)))].slice(0, BLOCK_TYPES.length)
    : undefined
  const accountIds = Array.isArray(source.paymentAccountSelection?.accountIds)
    ? [...new Set(source.paymentAccountSelection.accountIds.map(Number).filter((id) => Number.isInteger(id) && id > 0))].slice(0, 20)
    : undefined
  ...
  return compact({
    ...
    layout: Object.prototype.hasOwnProperty.call(source, 'layout') ? compact({
      spacing: SPACING_TOKENS.has(spacing) ? spacing : undefined,
      blockOrder,
    }) : undefined,
    paymentAccountSelection: Object.prototype.hasOwnProperty.call(source, 'paymentAccountSelection') ? compact({
      accountIds,
      showBankName: bool(source.paymentAccountSelection?.showBankName),
      showAccountName: bool(source.paymentAccountSelection?.showAccountName),
      showAccountNumber: bool(source.paymentAccountSelection?.showAccountNumber),
    }) : undefined,
  })
}
...
const normalizeDocumentPresentationConfig = (value) => {
  ...
  return { version: 2, shared: normalizeLayer(value.shared), documents }
}
