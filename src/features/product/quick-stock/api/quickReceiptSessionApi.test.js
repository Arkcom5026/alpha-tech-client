...
const patch = vi.fn();
const del = vi.fn();
const getAllSuppliers = vi.fn();
const parseApiError = vi.fn((error) => error);
const makeIdempotencyKey = vi.fn(() => 'cmd-test-001');

vi.mock('@/utils/apiClient', () => ({
  default: { get, post, patch, delete: del },
}));
vi.mock('@/utils/uiHelpers', () => ({ parseApiError }));
vi.mock('@/features/supplier/api/supplierApi', () => ({ getAllSuppliers }));
vi.mock('@/features/quickReceive/api/quickReceiveApi', () => ({ makeIdempotencyKey }));
...
describe('quickReceiptSessionApi', () => {
  ...
  it('loads and normalizes supplier rows', async () => {
    getAllSuppliers.mockResolvedValue({ data: [{ id: 1, name: 'Supplier A' }] });
  ...
  it('creates a resumable draft through the receipt collection endpoint', async () => {
    post.mockResolvedValue({ data: { data: { id: 77, status: 'DRAFT' } } });
    ...
    expect(post).toHaveBeenCalledWith('quick-stock/receipts', payload);
  });

  it('sends one-shot completion with an idempotency key', async () => {
    post.mockResolvedValue({ data: { data: { id: 88, status: 'COMPLETED' } } });
  ...
  it('finalizes a server draft with an idempotency key', async () => {
    post.mockResolvedValue({ data: { data: { id: 77, status: 'COMPLETED' } } });
  ...
  it('preserves parsed API errors', async () => {
    ...
    expect(parseApiError).toHaveBeenCalledWith(failure);
  });
});
