...
const TemplateImageGalleryPanel = ({ template }) => {
  ...
  const [pendingDeleteImage, setPendingDeleteImage] = React.useState(null);
  const {
    isUploadingImage,
  ...
  return (
    <>
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-col gap-2">
          ...
          <button
            ...
            className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white transition hover:bg-emerald-600 disabled:opacity-60"
          >
              ...
              return (
                <div key={image.id || image.public_id || url} className="overflow-hidden rounded-2xl border border-slate-100 bg-slate-50">
                  <div className="relative aspect-square">
                    ...
                    {image.isCover && (
                      <span className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-black text-emerald-600">Cover</span>
                    )}
                  </div>
      ...
      <ConfirmActionDialog
        ...
        title="ลบรูป Template"
        description="รูปนี้จะถูกลบออกจาก Template Catalog และไม่สามารถย้อนกลับได้"
        confirmLabel="ลบรูป"
        intent="destructive"
