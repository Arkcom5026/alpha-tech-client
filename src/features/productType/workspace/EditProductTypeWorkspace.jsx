  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (mounted) {
          const message = parseApiError(err) || 'ไม่สามารถโหลดข้อมูลประเภทสินค้าได้';
          setErrorMsg(message);
          feedback.actionError(err, message, 'product-type:edit:load:error');
        }
      } finally {
        ...
        <div className="w-full max-w-3xl">
          ...
          <div className="mt-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" className="rounded-lg border border-zinc-300 bg-white px-3 py-1.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50" onClick={() => navigate(-1)}>ย้อนกลับ</button>
              <Link className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-700 hover:bg-blue-100" to={listPath}>กลับไปหน้ารายการ</Link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="p-6 w-full flex flex-col items-center">
      <div className="w-full max-w-3xl">
        ...
        <div className="border rounded-xl p-4 shadow-sm bg-white dark:bg-zinc-900">
          {isFetching && <div className="text-sm text-zinc-600 dark:text-zinc-300">กำลังโหลดข้อมูล...</div>}
          {!isFetching && formData && (
            <ProductTypeForm
              mode="edit"
