...
const PurchaseStatusPanel = ({ eyebrow = 'สถานะงานจัดซื้อ', title, description, tone = 'neutral', actionLabel, onAction }) => {
  const tones = {
  ...
  return (
    <div className={`rounded-2xl border p-5 ${tones[tone] || tones.neutral}`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {onAction && <PurchaseWorkspaceButton onClick={onAction}>{actionLabel || 'ดูรายละเอียด'}</PurchaseWorkspaceButton>}
      </div>
    </div>
  );
};
