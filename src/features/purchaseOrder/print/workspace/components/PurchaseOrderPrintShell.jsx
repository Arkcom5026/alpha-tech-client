...
const purchaseOrderFillerRowCount = (lines = []) => {
  const occupiedRows = Math.max(Array.isArray(lines) ? lines.length : 0, 1);
  return Math.max(0, PURCHASE_ORDER_TABLE_TARGET_ROWS - occupiedRows);
};
    ...
    <div className="purchase-order-print-shell min-h-screen bg-slate-100 px-4 py-6 print:min-h-0 print:bg-white print:p-0">
      ...
      <article ref={printRef} className="print-area purchase-order-a4-page relative bg-white text-[14px] text-black shadow-sm">
        ...
        <table className="w-full table-fixed border-collapse border border-black text-[13px] leading-tight">
          ...
          <tbody>
            {lines.length === 0 ? <tr><td colSpan={5} className="h-[24px] border border-black p-2 text-center">ไม่มีรายการสินค้า</td></tr> : lines.map((line, idx) => (
              <tr key={line.id ?? idx}>
                <td className="h-[24px] border border-black px-1 py-1 text-center">{idx + 1}</td>
                <td className="h-[24px] border border-black px-2 py-1">{line.name}</td>
                <td className="h-[24px] border border-black px-1 py-1 text-center">{line.quantity.toLocaleString('th-TH')}</td>
                <td className="h-[24px] border border-black px-2 py-1 text-right tabular-nums">{formatPurchaseOrderMoney(line.costPrice)} ฿</td>
                <td className="h-[24px] border border-black px-2 py-1 text-right tabular-nums">{formatPurchaseOrderMoney(line.lineTotal)} ฿</td>
              </tr>
            ))}
            {Array.from({ length: fillerRowCount }).map((_, idx) => (
              <tr key={`purchase-order-filler-${idx}`} className="purchase-order-table-filler-row" aria-hidden="true">
