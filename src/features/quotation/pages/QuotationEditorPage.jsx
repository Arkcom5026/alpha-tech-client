...
import { FileText, PackagePlus, Search, UserRound } from 'lucide-react';
    ...
    <div className="mx-auto w-full max-w-[1380px] space-y-4 p-4 text-slate-800">
                    ...
                    return (
                      <button key={group.productId} type="button" disabled={Boolean(productSavingId)} onClick={() => addProductHelper(group)} className="block w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-left hover:border-teal-300 hover:bg-teal-50 disabled:opacity-50">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                          <span className="rounded-full bg-emerald-50 px-2 py-1 font-semibold text-emerald-800">พร้อมขาย: {group.hasAvailability ? `${group.availableQuantity} ชิ้น` : 'ไม่พบจำนวนจากผลค้นหา'}</span>
      ...
      <section data-testid="quotation-intake-overview" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-2 border-b border-slate-100 pb-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">ปรับจำนวนและราคาเพื่อประเมินยอดได้ทันที ส่วนรายละเอียดข้อความและ Manual ได้จากหน้าเอกสารโดยตรง</p>
          </div>
          ...
          <table className="min-w-[1120px] w-full text-sm">
            <thead className="bg-slate-50 text-xs text-slate-500">
              <tr>
                ...
                <th className="w-24 px-3 py-2.5 text-center">จำนวน</th>
                <th className="w-20 px-3 py-2.5 text-center">หน่วย</th>
                <th className="w-28 px-3 py-2.5 text-right">ราคาปัจจุบัน</th>
                <th className="w-32 px-3 py-2.5 text-center">ปรับราคา (+/-)</th>
                <th className="w-28 px-3 py-2.5 text-right">ราคาหลังปรับ</th>
                <th className="w-28 px-3 py-2.5 text-right">จำนวนเงิน</th>
                <th className="w-24 px-3 py-2.5 text-center">บันทึก</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {!quotationItems.length ? (
                <tr><td colSpan="10" className="px-3 py-8 text-center text-sm text-slate-500">ยังไม่มีรายการ — จะเลือกสินค้าจากด้านบน หรือเพิ่ม Manual บนหน้าเอกสารก็ได้</td></tr>
                ...
                return (
                  <tr key={line.id} className="align-top">
                    ...
                    <td className="px-3 py-3 text-right font-semibold tabular-nums text-teal-800">{money(adjustedPrice)}</td>
                    <td className="px-3 py-3 text-right font-semibold tabular-nums">{money(previewAmount)}</td>
                    <td className="px-2 py-2 text-center"><button data-testid={`quotation-intake-save-${line.id}`} type="button" disabled={!editable || Boolean(lineSavingId)} onClick={() => saveLineQuickEdit(line)} className="h-9 rounded-lg bg-teal-700 px-3 text-xs font-semibold text-white hover:bg-teal-800 disabled:opacity-50">{lineSavingId === line.id ? 'กำลังบันทึก' : 'บันทึก'}</button></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-teal-200 bg-teal-50/50 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">ปรับจำนวนและราคาเบื้องต้นได้จากหน้านี้ ส่วนรายละเอียดเอกสารจัดทำต่อบน Document Workspace โดยตรง</p>
        </div>
