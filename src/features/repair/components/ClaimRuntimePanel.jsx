...
const ClaimRuntimePanel = ({ claim, submitting, onTransition, onOpenRepair }) => {
  ...
  const canSubmit = Boolean(
    selectedAction &&
    (!requiresReason || draft.note.trim()) &&
    (!resolving || draft.resolution) &&
  ...
  const submit = () => onTransition({
    ...
    replacementStockItemId: resolutionRequiresReplacement
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              {claim.claimAsset?.displayName || claim.repairJob?.deviceModel || 'รายการเคลม'}
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Info label="ผู้จำหน่าย" value={claim.supplier?.name} />
          ...
          <Info label="Serial" value={claim.claimAsset?.serialNumber} />
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
            ...
            <>
              <div className="mt-4 grid gap-3">
                {nextStatuses.map((status) => {
                  ...
                  const active = selectedAction === status;
                  return (
                    <button
                      key={status}
                      type="button"
                      onClick={() => setSelectedAction(status)}
                      className={`rounded-xl border p-4 text-left ${active ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white'}`}
                    >
                ...
                <div className="mt-5 rounded-2xl border border-indigo-100 bg-slate-50 p-4">
                  ...
                  {resolving ? (
                    <div className="mt-3 space-y-3">
                      <select value={draft.resolution} onChange={(e) => setDraft((v) => ({ ...v, resolution: e.target.value }))} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3">
                        <option value="">เลือกผลการเคลม</option>
                        {RESOLUTION_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                      </select>
                      {resolutionRequiresReplacement ? (
                        <input value={draft.replacementStockItemId} onChange={(e) => setDraft((v) => ({ ...v, replacementStockItemId: e.target.value }))} inputMode="numeric" placeholder="รหัส StockItem สินค้าทดแทน" className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />
                      ) : null}
                      {resolutionRequiresCredit ? (
                        <input value={draft.creditAmount} onChange={(e) => setDraft((v) => ({ ...v, creditAmount: e.target.value }))} type="number" min="0" placeholder="ยอดเครดิต" className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />
                      ) : null}
                  ...
                  <button type="button" disabled={submitting || !canSubmit} onClick={submit} className="mt-3 w-full rounded-xl bg-indigo-700 px-5 py-3 font-black text-white disabled:opacity-40">
                    {submitting ? 'กำลังบันทึก...' : `ยืนยัน: ${ACTION_COPY[selectedAction]?.[0] || CLAIM_LABELS[selectedAction]}`}
                  </button>
                </div>
              ) : null}
            </>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <h3 className="mt-1 text-lg font-black text-slate-950">ประวัติการเคลม</h3>
          <div className="mt-4 space-y-4">
            {(claim.events || []).length ? claim.events.map((event) => (
              <div key={event.id} className="relative border-l-2 border-indigo-200 pl-4">
                ...
                <p className="font-black text-slate-900">{CLAIM_LABELS[event.status] || event.status}</p>
                <p className="mt-1 text-xs text-slate-500">{formatDateTime(event.occurredAt)}{event.performedByName ? ` · ${event.performedByName}` : ''}</p>
                {event.note ? <p className="mt-2 whitespace-pre-wrap text-sm text-slate-700">{event.note}</p> : null}
              </div>
