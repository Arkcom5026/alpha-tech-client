...
const RepairJobSummary = ({ job }) => {
  const deviceName = job.repairAsset?.displayName || job.assetDescription || job.deviceModel;
  const technicalModel = job.repairAsset?.model || job.device?.model || null;
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Item label="ชื่ออุปกรณ์" value={deviceName} />
        <Item label="รุ่น / Model" value={technicalModel} />
