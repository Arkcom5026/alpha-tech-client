    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40" role="dialog" aria-modal="true" aria-label="คู่มือคืนสินค้า">
      ...
      <aside className="h-full w-full max-w-xl overflow-y-auto bg-white shadow-2xl">
        <header className="sticky top-0 z-10 flex items-start justify-between border-b bg-white px-5 py-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-orange-600">Sale Return Help</p>
          ...
          <button type="button" className="rounded-xl border px-3 py-2 font-bold text-slate-700" onClick={onClose}>
            ปิด
          </button>
        </header>
        ...
        <div className="space-y-5 p-5">
          <section className="rounded-2xl border border-orange-200 bg-orange-50 p-4">
            <h3 className="font-black text-orange-900">ตรวจด่วนก่อนยืนยัน</h3>
          ...
          {SALE_RETURN_HELP_SECTIONS.map((section) => (
            <section key={section.id} className="rounded-2xl border bg-white p-4 shadow-sm">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-700">
                {section.items.map((item) => (
                  <li key={item} className="flex gap-2"><span className="text-orange-500">•</span><span>{item}</span></li>
                ))}
              </ul>
            </section>
          ))}
