...
const OnlineProductVisibilityDashboardPage = () => {
  ...
  return <div className="space-y-6 p-4 md:p-6">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2">{STATUS_OPTIONS.map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-xl px-3 py-2 text-sm font-bold ${filter === value ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600'}`}>{label}</button>)}</div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหาชื่อสินค้า บาร์โค้ด แบรนด์ หรือหมวดหมู่" className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm lg:max-w-md" /></div></div>
      <div className="divide-y divide-slate-100">{items.length ? items.map((item) => <article key={item.productId} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_160px_130px_110px] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-black text-slate-900">{item.name}</h3><StatusBadge status={item.status} /></div><p className="mt-1 text-xs text-slate-500">{[item.barcode, item.brand?.name, item.productType?.name, item.category?.name].filter(Boolean).join(' · ')}</p><div className="mt-3 flex flex-wrap gap-2">{(item.reasons || []).length ? item.reasons.map((reason) => <span key={reason} className={`rounded-lg px-2.5 py-1 text-xs font-semibold ${reason === 'OUT_OF_STOCK' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>{REASON_LABELS[reason] || reason}</span>) : <span className="rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">ผ่านทุกเงื่อนไขออนไลน์</span>}</div></div><div><p className="text-xs font-bold text-slate-400">ราคาออนไลน์</p><p className="mt-1 text-xl font-black">{item.priceOnline > 0 ? `฿${Number(item.priceOnline).toLocaleString('th-TH')}` : 'ยังไม่กำหนด'}</p></div><div><p className="text-xs font-bold text-slate-400">สต๊อกพร้อมขาย</p><p className="mt-1 text-xl font-black">{Number(item.availableQuantity || 0).toLocaleString('th-TH')}</p></div><button type="button" onClick={() => openEditor(item)} className="rounded-xl bg-blue-600 px-3 py-2 text-sm font-bold text-white">จัดการ</button></article>) : <div className="p-10 text-center text-sm text-slate-500">ไม่พบสินค้าที่ตรงกับตัวกรอง</div>}</div>
    </section>
    {editing ? <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 p-4"><div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl"><h2 className="text-xl font-black text-slate-900">จัดการสินค้าออนไลน์</h2><p className="mt-1 text-sm text-slate-500">{editing.name}</p><div className="mt-5 grid gap-4"><label className="text-sm font-bold text-slate-700">ราคาออนไลน์<input type="number" min="0" step="0.01" value={form.priceOnline} onChange={(e) => setForm((c) => ({ ...c, priceOnline: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3" /></label><label className="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700"><span>เปิดใช้งานราคาของร้าน</span><input type="checkbox" checked={form.isActive} onChange={(e) => setForm((c) => ({ ...c, isActive: e.target.checked }))} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-slate-700">เริ่มใช้ราคา<input type="date" value={form.effectiveDate} onChange={(e) => setForm((c) => ({ ...c, effectiveDate: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3" /></label><label className="text-sm font-bold text-slate-700">สิ้นสุดราคา<input type="date" value={form.expiredDate} onChange={(e) => setForm((c) => ({ ...c, expiredDate: e.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 px-4 py-3" /></label></div></div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setEditing(null)} className="rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold">ยกเลิก</button><button type="button" onClick={saveEditor} className="rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white">บันทึกและตรวจใหม่</button></div></div></div> : null}
  </div>;
};
