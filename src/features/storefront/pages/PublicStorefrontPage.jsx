...
const PublicStorefrontPage = () => {
  ...
  const renderSection = (section) => {
    if (section.type === 'HERO') {
      return <section key={section.id} className="overflow-hidden rounded-3xl p-8 shadow-sm md:p-12" style={{ background: tokens.brandAccent }}><p className="text-sm font-bold uppercase tracking-[0.2em] text-slate-800/70">ยินดีต้อนรับ</p><h2 className="mt-3 max-w-3xl text-3xl font-black leading-tight text-slate-950 md:text-5xl">เลือกสินค้าที่ใช่ จากร้านที่คุณไว้วางใจ</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-800/75">เลือกซื้อสินค้าจากสต๊อกของร้านโดยตรง พร้อมข้อมูลราคาและสถานะพร้อมจำหน่ายล่าสุด</p></section>;
    }
    if (section.type === 'FEATURED_PRODUCTS') {
      return <section key={section.id}><div className="mb-4 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Featured</p><h2 className="mt-1 text-2xl font-black">สินค้าแนะนำ</h2></div><span className="text-sm text-slate-500">{featured.length} รายการ</span></div>{featured.length ? <div className="grid gap-5 md:grid-cols-3">{featured.map((product) => <ProductCard key={product.id} product={product} shopSlug={shopSlug} compact />)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 p-8 text-center text-slate-500">ยังไม่มีสินค้าแนะนำที่พร้อมจำหน่าย</div>}</section>;
    }
    if (section.type === 'PRODUCT_GRID') {
      return <section key={section.id}><div className="mb-4 flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Catalog</p><h2 className="mt-1 text-2xl font-black">สินค้าทั้งหมด</h2></div><span className="text-sm text-slate-500">{products.length} รายการ</span></div>{products.length ? <div className={`grid gap-5 ${experience.layoutPreset === 'catalog-list' ? 'md:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>{products.map((product) => <ProductCard key={product.id} product={product} shopSlug={shopSlug} />)}</div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 p-8 text-center text-slate-500">ร้านยังไม่มีสินค้าที่เปิดขายออนไลน์</div>}</section>;
    }
    if (section.type === 'FULFILLMENT') {
      return <section key={section.id} className="grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-2xl">🏪</p><h2 className="mt-3 text-xl font-black">รับสินค้าที่ร้าน</h2><p className="mt-2 text-sm leading-6 text-slate-600">{storefront.fulfillment?.pickup?.enabled ? storefront.fulfillment.pickup.instruction || 'รับสินค้าที่หน้าร้าน' : 'ร้านนี้ยังไม่เปิดรับสินค้าที่ร้าน'}</p></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-2xl">🚚</p><h2 className="mt-3 text-xl font-black">จัดส่งสินค้า</h2><p className="mt-2 text-sm leading-6 text-slate-600">{storefront.fulfillment?.delivery?.enabled ? storefront.fulfillment.delivery.instruction || 'ร้านมีบริการจัดส่งสินค้า' : 'ร้านนี้ยังไม่เปิดบริการจัดส่ง'}</p></div></section>;
    }
    if (section.type === 'CONTACT') {
      return <section key={section.id} className="rounded-3xl p-8 text-white shadow-sm" style={{ background: tokens.brandPrimary }}><p className="text-xs font-bold uppercase tracking-[0.2em] text-white/70">Contact</p><h2 className="mt-2 text-3xl font-black">ติดต่อร้าน</h2><div className="mt-5 grid gap-4 text-sm md:grid-cols-2"><div><p className="text-white/60">เบอร์โทร</p><p className="mt-1 text-lg font-bold">{storefront.contactPhone || 'ยังไม่ได้ระบุ'}</p></div><div><p className="text-white/60">ที่อยู่</p><p className="mt-1 text-lg font-bold">{storefront.address || 'ยังไม่ได้ระบุ'}</p></div></div></section>;
    }
    return null;
  };
  ...
  return (
    <main className="min-h-screen" style={{ background: tokens.surface, color: tokens.text }}>
      <header className="sticky top-0 z-10 border-b border-white/10 text-white shadow-sm" style={{ background: tokens.brandPrimary }}><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4"><div><p className="text-xs text-white/65">/{storefront.slug}</p><h1 className="text-xl font-black md:text-2xl">{storefront.name}</h1></div><Link to="/" className="rounded-xl bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur hover:bg-white/20">Marketplace</Link></div></header>
