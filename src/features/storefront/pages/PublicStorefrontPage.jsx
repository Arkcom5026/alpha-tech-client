...
const PublicStorefrontPage = () => {
  ...
  const [productState, setProductState] = useState({ loading: true, items: [], facets: { categories: [], brands: [] }, pagination: { page: 1, pageSize: 24, total: 0, totalPages: 0 }, error: '' });
  ...
  useEffect(() => {
    ...
    apiClient.get(`/sales/storefronts/${slug}`, { skipAuthBootstrap: true })
      ...
      .catch((error) => {
        ...
        if (status === 404 || code === 'STOREFRONT_NOT_FOUND') return setStoreState({ loading: false, storefront: null, notFound: true, error: '' });
        setStoreState({ loading: false, storefront: null, notFound: false, error: error?.response?.data?.message || 'ไม่สามารถโหลดหน้าร้านได้ในขณะนี้' });
      });
    return () => { active = false; };
  }, [shopSlug]);
  ...
  useEffect(() => {
    ...
    apiClient.get(`/sales/storefronts/${encodeURIComponent(shopSlug || '')}/products?${params.toString()}`, { skipAuthBootstrap: true })
      ...
      .catch((error) => {
        if (!active) return;
        setProductState((current) => ({ ...current, loading: false, items: [], error: error?.response?.data?.message || 'ไม่สามารถค้นหาสินค้าได้' }));
      });
    return () => { active = false; };
  ...
  if (storeState.loading) return <main className="grid min-h-screen place-items-center bg-slate-50 p-8 text-slate-600">กำลังโหลดหน้าร้าน...</main>;
  if (storeState.error) return <main className="grid min-h-screen place-items-center bg-slate-50 p-8 text-center text-red-700">{storeState.error}</main>;
  if (storeState.notFound || !storeState.storefront) return <main className="grid min-h-screen place-items-center bg-slate-50 p-6"><section className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><div className="text-5xl">🏪</div><h1 className="mt-5 text-2xl font-bold">ร้านนี้กำลังจัดเตรียมหน้าร้าน</h1><p className="mt-2 text-sm text-slate-500">เจ้าของร้านกำลังเพิ่มข้อมูลและสินค้า โปรดกลับมาเยี่ยมชมอีกครั้งในภายหลัง</p><Link to="/" className="mt-6 inline-flex rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white">กลับหน้า Marketplace</Link></section></main>;
  ...
  const discoveryControls = (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <form className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_180px_180px_auto]" onSubmit={(event) => { event.preventDefault(); updateQuery({ q: searchText.trim() }); }}>
      ...
      {hasFilters ? <button type="button" onClick={() => { setSearchText(''); setSearchParams({}); }} className="mt-3 text-sm font-bold text-blue-700 hover:underline">ล้างคำค้นและตัวกรอง</button> : null}
    </section>
  );
  ...
  const renderSection = (section) => {
    if (section.type === 'HERO') return <section key={section.id} className="overflow-hidden rounded-3xl p-8 shadow-sm md:p-12" style={{ background: tokens.brandAccent }}><p className="text-sm font-bold uppercase tracking-[0.2em] text-slate-800/70">ยินดีต้อนรับ</p><h2 className="mt-3 max-w-3xl text-3xl font-black leading-tight text-slate-950 md:text-5xl">เลือกสินค้าที่ใช่ จากร้านที่คุณไว้วางใจ</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-800/75">ค้นหาและเลือกซื้อสินค้าจากสต๊อกของร้านโดยตรง พร้อมราคาและสถานะล่าสุด</p></section>;
    ...
    if (section.type === 'PRODUCT_GRID') return <section key={section.id}>{discoveryControls}<div className="mb-4 mt-7 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Catalog</p><h2 className="mt-1 text-2xl font-black">สินค้าทั้งหมด</h2></div><span className="text-sm text-slate-500">{pagination.total.toLocaleString('th-TH')} รายการ</span></div>{productState.loading ? <div className="rounded-2xl border border-dashed p-10 text-center text-slate-500">กำลังค้นหาสินค้า...</div> : productState.error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center text-red-700">{productState.error}</div> : products.length ? <><div className={`grid gap-5 ${experience.layoutPreset === 'catalog-list' ? 'md:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}`}>{products.map((p) => <ProductCard key={p.id} product={p} shopSlug={shopSlug} />)}</div>{paginationControls}</> : <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-slate-500"><p className="font-bold text-slate-700">ไม่พบสินค้าที่ตรงกับเงื่อนไข</p><button type="button" onClick={() => { setSearchText(''); setSearchParams({}); }} className="mt-3 font-bold text-blue-700">ล้างตัวกรอง</button></div>}</section>;
