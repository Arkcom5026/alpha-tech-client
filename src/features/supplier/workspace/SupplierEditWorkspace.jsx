    ...
    <div className="max-w-4xl mx-auto p-2 md:p-4 space-y-3 animate-fadeIn text-xs md:text-sm">
      <div className="bg-white border border-slate-200 p-3 rounded-xl shadow-sm flex items-center justify-between gap-2 select-none">
        ...
        <div className="flex items-center gap-1.5">
          <TooltipProvider><Tooltip><TooltipTrigger asChild><Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><button type="button" className="flex items-center gap-1 h-7 px-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/60 rounded-lg text-[11px] font-black transition-all shadow-sm"><Trash className="w-3 h-3" /><span>ลบผู้ขาย</span></button></DialogTrigger><DialogContent className="sm:max-w-[425px]"><DialogHeader><p className="text-sm font-black text-slate-900">ยืนยันสัญญาณถอนถอนข้อมูลคู่ค้าออกจากคลัง</p><p className="text-xs text-slate-400 font-medium pt-1 leading-relaxed">คุณแน่ใจหรือไม่ว่าต้องการลบผู้ขายรายนี้? ระบบจะทำการล้างข้อมูลบัญชีคู่ค้าออกจากฐานข้อมูลส่วนกลางถาวรและไม่สามารถย้อนกลับลูปข้อมูลได้</p></DialogHeader><DialogFooter className="mt-4 gap-2 sm:gap-0"><Button variant="outline" size="sm" onClick={() => setOpen(false)} className="text-xs font-bold h-7 rounded-lg">ยกเลิกขั้นตอน</Button><Button variant="destructive" size="sm" onClick={handleDelete} className="text-xs font-black bg-rose-600 hover:bg-rose-700 h-7 rounded-lg">ยืนยันลบถาวร</Button></DialogFooter></DialogContent></Dialog></TooltipTrigger><TooltipContent side="bottom" className="text-[10px] font-black bg-slate-900 text-white border-none rounded p-1.5 shadow-md">⚠️ การล้างบัญชีรายชื่อผู้ขายจะไม่สามารถย้อนกลับลูปข้อมูลได้</TooltipContent></Tooltip></TooltipProvider>
          <button type="button" onClick={() => navigate(paths.view(id))} className="flex items-center gap-1 h-7 px-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-lg text-[11px] font-black transition-all shadow-sm"><ArrowLeft className="w-3 h-3" /><span>ยกเลิก</span></button>
        </div>
      </div>
