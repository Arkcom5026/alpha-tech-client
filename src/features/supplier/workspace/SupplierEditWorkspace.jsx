...
const SupplierEditWorkspace = () => {
  ...
  if (loading) {
    return <div className="text-center py-12 font-bold text-slate-400 italic tracking-wide">⏳ กำลังเรียกถอดรหัสฟื้นคืนระบบข้อมูลคู่ค้า...</div>;
  }
  if (!defaultValues) {
    return <p className="text-center text-rose-500 font-black py-10 select-none">❌ ไม่พบข้อมูลรายชื่อบริษัทคู่ค้าในระบบทะเบียนคลัง</p>;
  }
  if (!selectedBranchId) {
    return <p className="text-center text-slate-400 font-bold py-10 select-none">📭 ยังไม่ได้ทำการเลือกสาขาผู้ปฏิบัติงานในระบบ</p>;
  }
    ...
    <div className="max-w-4xl mx-auto p-2 md:p-4 space-y-3 animate-fadeIn text-xs md:text-sm">
      <div className="bg-white border border-slate-200 p-3 rounded-xl shadow-sm flex items-center justify-between gap-2 select-none">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            type="button"
            onClick={() => navigate(paths.view(id))}
