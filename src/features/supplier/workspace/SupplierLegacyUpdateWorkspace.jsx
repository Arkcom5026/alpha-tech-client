...
const SupplierLegacyUpdateWorkspace = () => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        ...
        feedback.actionError(error, 'โหลดข้อมูลผู้ขายไม่สำเร็จ', 'supplier:legacy:load:error');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleUpdate = async (formData) => {
    ...
    try {
      ...
      feedback.actionSuccess('บันทึกการแก้ไขผู้ขายเรียบร้อยแล้ว', 'supplier:legacy:update:success');
      navigate(paths.list);
    } catch (error) {
      feedback.actionError(error, 'บันทึกการแก้ไขผู้ขายไม่สำเร็จ', 'supplier:legacy:update:error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-2 md:p-4 space-y-3 animate-fadeIn text-xs md:text-sm">
      <div className="flex items-center justify-between bg-white border border-slate-200 p-3 rounded-xl shadow-sm select-none">
        ...
        <button type="button" disabled={loading} onClick={() => navigate(paths.list)} className="flex items-center gap-1 h-7 px-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 rounded-lg text-[11px] font-black transition-all shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"><ArrowLeft className="w-3 h-3" /><span>กลับหน้าทะเบียน</span></button>
      </div>
