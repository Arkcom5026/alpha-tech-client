...
const SupplierAdvancePaymentForm = ({ supplier }) => {
  ...
  const isSubmitButtonDisabled = interactionBusy || Number.isNaN(parsedAmountForValidation) || parsedAmountForValidation <= 0;
    ...
    <div>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 items-start gap-6 md:grid-cols-3">
          ...
          <PaymentMethodInput label="จำนวนเงิน" value={formData.amount} onChange={handleAmountChange} onBlur={handleAmountBlur} colorClass={{ CASH: 'cash', TRANSFER: 'transfer', CHEQUE: 'cheque' }[formData.method]} disabled={interactionBusy} />
        </div>
        ...
        <div><label htmlFor="note" className="mb-1 block text-sm font-medium text-gray-700">หมายเหตุ (ถ้ามี)</label><textarea disabled={interactionBusy} id="note" name="note" value={formData.note} onChange={handleChange} className="w-full rounded-md border-gray-300 shadow-sm disabled:opacity-60" rows="3" /></div>
