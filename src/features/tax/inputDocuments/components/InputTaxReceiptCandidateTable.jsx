...
const InputTaxReceiptCandidateTable = ({
  ...
  <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="border-b border-slate-200 p-4">
      <h2 className="font-black text-slate-900">ใบรับสินค้า</h2>
      <p className="text-xs text-slate-500">เลือกได้หลายใบ แต่ต้องเป็น Supplier เดียวกัน</p>
    </div>
      ...
      <table className="min-w-full text-sm">
        <thead className="bg-slate-50 text-left text-xs font-bold uppercase text-slate-500">
          <tr>
            ...
            <th className="p-3">Supplier</th><th className="p-3">วันที่รับ</th>
            <th className="p-3 text-right">ยอดใบรับ</th><th className="p-3 text-right">คงเหลือ</th>
            ...
            return (
              <React.Fragment key={key}>
                <tr className={selection ? 'bg-blue-50/50' : ''}>
                  ...
                  <td className="p-3"><p className="font-semibold text-slate-800">{receipt.supplierName}</p><p className="text-xs text-slate-500">#{receipt.supplierId}</p></td>
                  ...
                  <td className="p-3"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${badge[receipt.linkState] || 'bg-slate-100 text-slate-600'}`}>{linkStateLabel[receipt.linkState] || receipt.linkState}</span></td>
                </tr>
                {selection && (
                  <tr className="bg-blue-50/30">
                    ...
                    <td colSpan={6} className="p-3">
                      <div className="grid gap-2 sm:grid-cols-3">
                        {[
                          ['allocatedSubtotal', 'มูลค่าก่อน VAT'],
                          ['allocatedVatAmount', 'VAT'],
                          ['allocatedTotalAmount', 'ยอดรวมจัดสรร'],
                        ].map(([field, label]) => (
