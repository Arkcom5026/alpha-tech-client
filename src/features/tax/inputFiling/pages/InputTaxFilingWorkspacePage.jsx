...
import { ArrowLeft, CheckCircle2, FileCheck2, RefreshCw, ShieldCheck, XCircle } from 'lucide-react';
...
const eligibilityText = (document) => {
  ...
  return reasons.map((code) => ELIGIBILITY_REASON_TH[code] || 'ยังไม่พร้อมนำเข้าชุด').join(' · ') || 'ยังไม่พร้อมนำเข้าชุด';
};
...
const InputTaxFilingWorkspacePage = () => {
  ...
  const prepare = async () => {
    if (!branchId || !taxPeriodId || submitting) return;
    setSubmitting(true);
    try {
        ...
        <>
          {!batch ? (
            <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h2 className="font-black text-amber-950">ยังไม่ได้เริ่มเตรียมชุดภาษีซื้อของรอบนี้</h2>
                  <p className="mt-1 text-sm text-amber-800">ระบบจะสร้างชุดแบบร่างสำหรับรวบรวมเฉพาะรายการภาษีซื้อที่มี authority และผ่านเงื่อนไขจากฝั่ง Server</p>
                </div>
            ...
            <section className={`rounded-3xl border p-5 ${summary.readyForTaxClosing ? 'border-emerald-200 bg-emerald-50' : 'border-blue-200 bg-blue-50/40'}`}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <div className="flex items-center gap-2"><ShieldCheck size={19} className={summary.readyForTaxClosing ? 'text-emerald-700' : 'text-blue-700'} /><h2 className="font-black text-slate-950">ชุดภาษีซื้อ #{batch.id} · {batch.status === 'DRAFT' ? 'กำลังจัดเตรียม' : 'บันทึกแล้ว'}</h2></div>
                ...
                {summary.readyForTaxClosing ? (
                  <button type="button" onClick={goReadiness} className="rounded-xl bg-emerald-700 px-5 py-3 font-black text-white">กลับไปตรวจความพร้อมภาษี</button>
                ) : (
          ...
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Metric label="ยังเหลือ" value={summary.remainingDocumentCount || 0} />
            <Metric label="สถานะการเตรียม" value={summary.readyForTaxClosing ? 'ครบแล้ว' : (batch ? 'กำลังจัดเตรียม' : 'ยังไม่เริ่ม')} />
          </section>

          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="border-b border-slate-200 px-4 py-3">
              <h2 className="font-black text-slate-950">รายการภาษีซื้อของรอบนี้</h2>
              <p className="mt-1 text-xs text-slate-500">ระบบใช้รายการ Input VAT ที่เป็น authority และให้ Server เป็นผู้ตัดสินว่านำเข้าชุดได้หรือไม่</p>
            </div>
            {documents.length === 0 ? (
              <p className="p-8 text-center text-sm font-semibold text-slate-500">รอบนี้ยังไม่มีรายการภาษีซื้อที่เป็น authority</p>
            ) : (
                ...
                {documents.map((document) => {
                  ...
                  return (
                    <article key={document.taxDocumentId} className="p-4">
                      <div className="grid gap-4 lg:grid-cols-[1.35fr_.8fr_.8fr_auto] lg:items-center">
                        <div>
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-500">มูลค่าก่อน VAT</p>
                          <p className="mt-1 font-black text-slate-900">{formatTaxMoney(document.subtotalAmount)}</p>
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-500">VAT ที่ใช้เป็น authority</p>
                          <p className="mt-1 font-black text-slate-900">{formatTaxMoney(document.vatAmount)}</p>
                        </div>
