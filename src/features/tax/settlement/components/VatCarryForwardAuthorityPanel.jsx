...
const VatCarryForwardAuthorityPanel = ({ branchId, taxPeriodId, onConfirmed }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setContext(data);
      setAmount(Number(data?.authority?.amount || 0).toFixed(2));
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[220px_1fr_auto] lg:items-end">
        ...
        <label className="text-sm font-bold text-slate-700">
          ...
          <input
            ...
            disabled={immutable || saving}
            placeholder="เช่น ยอดตาม ภ.พ.30 เดือนก่อน / ยอดเปิดระบบที่นักบัญชียืนยัน"
        ...
        <button type="button" onClick={confirm} disabled={immutable || saving || loading} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-700 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">
