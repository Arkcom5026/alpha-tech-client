...
const WithholdingTaxWorkspacePage = () => {
  ...
  const issue = async (group) => {
    ...
    if (!selectedForm) {
      toast.error('กรุณาเลือก ภ.ง.ด.3 หรือ ภ.ง.ด.53 สำหรับผู้รับเงินรายนี้');
      return;
    }
    await run(`cert:${first.taxExpenseId}`, () => issueWithholdingCertificate({
      branchId,
      taxPeriodId,
      taxExpenseId: first.taxExpenseId,
      formType: selectedForm,
    }), `ออกหนังสือรับรอง WHT ${first.expenseNumber} แล้ว`);
  };
  ...
  const submit = async (formType) => run(`submit:${formType}`, () => submitWithholdingFiling({
    branchId,
    taxPeriodId,
    formType,
    reference: references[formType],
  }), `บันทึกหลักฐานการยื่น ${formLabel(formType)} แล้ว`);
    ...
    <section className="space-y-5">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">Certificate → ภ.ง.ด.3 / ภ.ง.ด.53 preparation → manual filing evidence — ยังไม่ใช่ direct e-Filing</p>
            </div>
          </div>
      ...
      {loading ? <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-sm font-semibold text-slate-500">กำลังโหลด WHT workspace...</div> : data ? <>
        <section className="grid gap-3 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><p className="text-xs text-violet-700">ประเมิน WITHHELD แล้ว</p><p className="mt-1 text-2xl font-black text-violet-950">{summary.withheldItemCount || 0}</p></div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          <h2 className="font-black text-slate-900">หนังสือรับรองการหักภาษี ณ ที่จ่าย</h2>
          {expenseGroups.length === 0 ? <p className="mt-3 text-sm text-slate-500">ไม่มีรายการค่าใช้จ่ายที่มี WHT ในรอบนี้</p> : <div className="mt-3 space-y-3">
            {expenseGroups.map((group) => {
              ...
              return <div key={first.taxExpenseId} className="rounded-xl border border-slate-200 p-3">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <div className="font-black text-slate-900">{first.expenseNumber} · {first.counterpartyName}</div>
                    <div className="mt-1 text-xs text-slate-500">{group.length} รายการ · WHT ฿{money(group.reduce((sum, row) => sum + Number(row.withholdingTaxAmount || 0), 0))} · {certificateIssued ? `ใบรับรอง ${first.certificateNumber}` : 'ยังไม่ออกใบรับรอง'}</div>
                  </div>
                ...
                {!canIssue && <p className="mt-2 text-xs font-semibold text-amber-700">ต้องประเมิน WHT treatment เป็น WITHHELD และมียอด/อัตราครบก่อนออกใบรับรอง</p>}
              </div>;
            })}
          </div>}
        </section>
          ...
          {['PND3', 'PND53'].map((formType) => {
            ...
            return <div key={formType} className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button type="button" disabled={!!busyKey || filing?.status === 'SUBMITTED'} onClick={() => prepare(formType)} className="rounded-lg border border-blue-300 bg-blue-50 px-3 py-2 text-xs font-bold text-blue-800 disabled:opacity-40">เตรียม {formLabel(formType)}</button>
              </div>
              {filing?.status === 'PREPARED' && <div className="mt-3 space-y-2 rounded-xl border border-amber-200 bg-amber-50 p-3">
                <label className="block text-xs font-bold text-amber-900">เลขอ้างอิง/หลักฐานการยื่นภายนอก</label>
                <input value={references[formType] || ''} onChange={(event) => setReferences((current) => ({ ...current, [formType]: event.target.value }))} className="w-full rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm" placeholder="เช่น RD-ACK-2026-08-001" />
                <button type="button" disabled={!!busyKey || !String(references[formType] || '').trim()} onClick={() => submit(formType)} className="rounded-lg bg-amber-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-40">ยืนยันว่าดำเนินการยื่นภายนอกแล้ว</button>
              </div>}
