        ...
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
            ...
            {Object.values(TEMPLATE_CANDIDATE_STATUS).map((status) => (
              <button
                key={status}
                type="button"
                onClick={() => applyFilters({ ...filters, status, page: 1 })}
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <form onSubmit={handleSearch} className="grid gap-3 xl:grid-cols-[minmax(240px,1fr)_190px_150px_180px_140px_120px]">
              ...
              />
              <select value={filters.status} onChange={(event) => updateFilter('status', event.target.value)} className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-bold text-slate-700">
              ...
              <input value={filters.reviewerId} onChange={(event) => updateFilter('reviewerId', event.target.value.replace(/\D/g, ''))} placeholder="Reviewer ID" inputMode="numeric" className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-semibold" />
              <select value={filters.sortBy} onChange={(event) => updateFilter('sortBy', event.target.value)} className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-bold text-slate-700">
                {SORT_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
              <select value={filters.sortDirection} onChange={(event) => updateFilter('sortDirection', event.target.value)} className="min-h-11 rounded-2xl border border-slate-200 px-4 text-sm font-bold text-slate-700">
                <option value="desc">มาก → น้อย</option>
                <option value="asc">น้อย → มาก</option>
              </select>
              <button type="submit" disabled={loading} className="min-h-11 rounded-2xl bg-slate-950 px-5 text-sm font-black text-white transition hover:bg-orange-500 disabled:opacity-50">ค้นหา</button>
            </form>
          </section>
          ...
          <section className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <div className="hidden grid-cols-[80px_150px_1.4fr_1fr_1fr_150px] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-[11px] font-black uppercase tracking-[0.1em] text-slate-500 md:grid">
                <div>ID</div><div>Business Type</div><div>Source Product / Store</div><div>Target Template</div><div>Reviewer</div><div>Status / Updated</div>
              </div>
              {loading && candidates.length === 0 ? (
                <div className="p-10 text-center text-sm font-bold text-slate-500">กำลังโหลด Candidate...</div>
              ) : candidates.length === 0 ? (
                <div className="p-10 text-center text-sm font-bold text-slate-500">ไม่พบ Candidate ในกลุ่ม {getBusinessTypeLabel(filters.businessType)}</div>
              ) : (
                <div className="divide-y divide-slate-100">
                  {candidates.map((candidate) => (
                    <button key={candidate.id} type="button" onClick={() => navigate(`${detailBasePath}/${candidate.id}`)} className="grid w-full gap-2 px-4 py-4 text-left transition hover:bg-orange-50/60 md:grid-cols-[80px_150px_1.4fr_1fr_1fr_150px] md:gap-3">
                      <div className="text-sm font-black text-slate-800">#{candidate.id}</div>
                      <div><span className="inline-flex rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700">{getBusinessTypeLabel(candidate.businessType)}</span></div>
                      <div className="min-w-0">
                        <p className="truncate text-sm font-black text-slate-900">{candidate.sourceProductName || '-'}</p>
                        <p className="mt-1 truncate text-xs font-semibold text-slate-500">{candidate.sourceBranchName || '-'} · Product #{candidate.sourceProductId || '-'}</p>
                      </div>
                      ...
                      <div className="min-w-0 text-sm font-bold text-slate-600">
                        <p className="truncate">{candidate.reviewedByEmployee?.name || candidate.reviewerName || 'ยังไม่มี Reviewer'}</p>
                        <p className="mt-1 text-xs text-slate-400">ID {candidate.reviewedByEmployeeId || '-'}</p>
                      </div>
                      <div>
                        <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-black ${getStatusClass(candidate.status)}`}>{getCandidateStatusLabel(candidate.status)}</span>
              ...
              <div className="flex flex-col gap-3 border-t border-slate-100 px-4 py-4 text-sm font-bold text-slate-600 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-2">
                  <button type="button" disabled={page <= 1 || loading} onClick={() => handlePage(page - 1)} className="min-h-11 rounded-2xl border border-slate-200 px-4 disabled:opacity-40">ก่อนหน้า</button>
                  <button type="button" disabled={page >= totalPages || loading} onClick={() => handlePage(page + 1)} className="min-h-11 rounded-2xl border border-slate-200 px-4 disabled:opacity-40">ถัดไป</button>
                </div>
              </div>
            </div>
            ...
            <aside className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <h2 className="mt-2 text-lg font-black text-slate-900">{getBusinessTypeLabel(filters.businessType)}</h2>
              <div className="mt-4 space-y-3">
                {reviewerWorkload.length === 0 ? (
                  ...
                  <button key={item.reviewerId} type="button" onClick={() => applyFilters({ ...filters, reviewerId: String(item.reviewerId), page: 1 })} className="min-h-16 w-full rounded-2xl border border-slate-200 p-4 text-left transition hover:border-orange-300">
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-sm font-black text-slate-800">Reviewer #{item.reviewerId}</span>
                      <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-black text-blue-700">{item.total ?? item.count ?? 0}</span>
                    </div>
                  </button>
                ))}
              </div>
