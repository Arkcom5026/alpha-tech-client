      ...
      {loading && candidates.length === 0 ? (
        ...
        <div className="divide-y divide-slate-100">
          {candidates.map((candidate) => (
            <button key={candidate.id} type="button" onClick={() => onOpenCandidate(candidate.id)} className="grid w-full gap-2 px-4 py-4 text-left transition hover:bg-orange-50/60 md:grid-cols-[80px_150px_1.4fr_1fr_1fr_150px] md:gap-3">
    ...
    <aside className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-4 space-y-3">
        {reviewerWorkload.length === 0 ? (
          ...
          <button key={item.reviewerId} type="button" onClick={() => onReviewer(item.reviewerId)} className="min-h-16 w-full rounded-2xl border border-slate-200 p-4 text-left transition hover:border-orange-300">
