...
const rangePrint = read('src/features/barcode/pages/BarcodeRangePrintPage.jsx')
...
assert(rangePrint.includes('@page { margin: 0mm; size: A4; }'), 'Barcode range print must keep its edge-to-edge A4 utility geometry.')
...
assert(rangePrint.includes('.c39-barcode') && rangePrint.includes('C39HrP24DhTt'), 'Barcode range print must keep Code39 font authority.')
assert(
  !batch.includes('FullTaxA4Document')
    && !preview.includes('FullTaxA4Document')
    && !rangePrint.includes('FullTaxA4Document'),
  'Barcode utility sheets must remain module owned.'
)
