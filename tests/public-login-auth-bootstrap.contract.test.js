...
describe('public login auth bootstrap contract', () => {
  it('does not call refresh bootstrap on public auth routes without session evidence', () => {
    ...
    expect(app).toContain('isPublicAuthEntryPath(pathname)');
    ...
    const guardIndex = app.indexOf('if (isPublicAuthEntryPath(pathname) && !hasRecoverableSessionEvidence(state))');
