...
describe('StockItem receive runtime ownership contract', () => {
  ...
  it('keeps the compatibility store as a consumer instead of a receive transport owner', () => {
    const store = read('src/features/stockItem/store/stockItemStore.js');

    expect(store).toContain("from '../receive'");
    expect(store).toContain('receiveScannedStockItem');
    expect(store).toContain('receiveAllPendingStockItems');
    expect(store).not.toContain('receiveStockItem,');
    expect(store).not.toContain('receiveAllPendingNoSN,');
