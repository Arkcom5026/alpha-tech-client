...
const createGatewayRuntimeConfig = ({
  ...
  const normalizedEndpoint = requireText(endpoint, 'endpoint')
  const parsedEndpoint = new URL(normalizedEndpoint)
  if (!['ws:', 'wss:'].includes(parsedEndpoint.protocol)) {
    throw new TypeError('endpoint must use ws or wss protocol')
  }
  ...
  if (normalizedMaxDelay < normalizedInitialDelay) {
    throw new TypeError('reconnectMaxDelayMs must be >= reconnectInitialDelayMs')
  }
  ...
  return Object.freeze({
    ...
    physicalExecutionEnabled: physicalExecutionEnabled === true,
  })
}
...
const createGatewayRuntimeConfigFromEnv = (env = process.env) => createGatewayRuntimeConfig({
  ...
  physicalExecutionEnabled: env.ALPHA_DEVICE_GATEWAY_PHYSICAL_EXECUTION === '1',
})
